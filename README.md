...
![изображение](https://github.com/user-attachments/assets/44b3e7c1-a577-4ead-a1c1-c169a7f4b12a)
...
Можно запланировать одноразовое обновление контейнеров по Московскому времени c помощью **в**

Обновляются командой `cD /opt/remnawave и docker compose down && docker compose pull && docker compose up -d`

Лог запуска идет в телеграм чат через бот, указанные в /opt/remnawave/.env

*Есть.*

![изображение](https://github.com/user-attachments/assets/0c33c20f-a120-456b-bdea-d7039c30e0be)
...
![изображение](https://github.com/user-attachments/assets/8f0c7183-56ab-4337-afad-0a785f1daae7)
...
Добавляет SSH ключ доступа (нужно вставить из буфера), настраивает SSH на работу только по ключам. Пользователя не меняет, работает из-под `корень`

![изображение](https://github.com/user-attachments/assets/47ea81de-9c52-4021-b988-c6b83a2fca56)


Запуск скрипта:

```
com/omchick33/remnawave-script/refs/heads/main/sh&s-s/root/sh&s&bs/root/sh.
```
...
## fail2ban на SSH
Скрипт установит fail2ban и настроет его тюрьму на ssh, автоматически определяя источник логов - `journalctl` или `/война/народ/восемь.`
