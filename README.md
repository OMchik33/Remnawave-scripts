...
> Внимание! На некоторых редких хостингах SSH служба настроена так, что скрипт отрабатывает некорректно, поэтому следуйте инструкциям скрипта, а именно - после его отработки проверяйте доступ SSH по ключу в другой вкладке SSH клиента, не закрывая основную, чтобы в случае проблем успеть откатиться из резервной копии.
